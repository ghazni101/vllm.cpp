...
`ACTIVE`. Position: 75.6 tok/s median at the T8 config (T8 evidence,
`docs/bench-evidence/gfx1100-tg200-t8-coop-rmsnorm-20260825.md`; GPU busy
11.14 ms/tok). Adopted levers: T5a shared quant-body vectorization (+23%),
T5b d128 f32-Q DecodeGqa arm (+13.5%), T6a cooperative GDN scan (+4.6%),
T6b cooperative attn preamble (+4.6%), T8 cooperative rmsnorm row
(+3.2%). Closed negative: T5c MMVQ nontemporal loads, T7 COALK
...
Next attack: the GDN latency trio (GdnScanCoop 0.730 + PostConv ~0.67 +
RmsNormGated 0.414 ≈ 1.81 ms/tok combined), then wvSplitKSml's 408 GB/s
vs the 598 reference. Owed before ANY default flip of the opt-in arms
(GQA4 / GDN_SCAN_COOP / PREAMBLE_COOP / RMSNORM_ROW_COOP):
