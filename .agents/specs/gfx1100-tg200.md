...
`ACTIVE`. Position: 84.3 tok/s median at the T11 config (evidence
...
gated norm (+2.6%), T10 warp postconv (+4.3%, 27.9->2.76us), T11 row-split
scan (+3.9% BIT-IDENTICAL, 30.4->9.57us). Closed negative: T5c MMVQ nontemporal loads, T7 COALK
...
Next attack: dispatch-gap audit (up to ~1.0 ms/tok), rmsnorm_row second
pass (+0.38), streaming micro-tuning. T12 gated-norm producer-quant fusion
