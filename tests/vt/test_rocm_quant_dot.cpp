...
// test-backend-ops.cpp:4277 band -- the f64-dequant ceiling for the CIQ
// keep-quant gate (KERNEL-QUANT-CIQ-GEMM-ROCM W1).
...
// KERNEL-QUANT-CIQ-GEMM-ROCM W1: the full ten-encoding table the CUDA
// sibling serves (test_cuda_quant_dot.cpp WeightCase table). Used by
// the CIQ provider gate cases below.
const WeightCase kCiqCases[] = {
...
TEST_CASE("ROCm keep-quant GEMM == CPU reference and f64 dequant (Q8_K family)") {
  ...
  for (const WeightCase& c : kCiqCases) {
...
TEST_CASE(
  ...
  CAPTURE(combos);
  CHECK(combos ==
        static_cast<int64_t>(std::size(kCiqCases) * std::size(kGroupedShapes)));
  CHECK(combos > 0);
  gpu.DestroyQueue(gq);
}
